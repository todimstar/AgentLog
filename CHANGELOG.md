...
## 0.1.0-SNAPSHOT - L18 Retry 与 Terminate（冻住之后谁来解冻 · 待提交）

日期：2026-08-16　出处：`[会话 ed69ff80]`（继承 L16/L17 会话 `3ad9ede8`）　设计简报：`docs/decisions/0008-l18-retry-and-terminate.md`

> **本课是整个项目第一次出现「机器有能力做、但故意不让它做」的环节。**
> 前 17 课都在教「怎么让机器自己搞定」，这一课教的是——**在哪里必须停下来等人**。
> 判据不是「人比机器聪明」，而是 **这一步需要的信息只存在于系统之外**（你还想不想继续写、能不能把那个 AI 叫回来）。

### Added

- 🔴 **写作前文端点**（`GET /agent/contribution-tickets/{code}/preceding-content`）——
  **主人在 L18 验收时发现的缺口**：他在等第 2 棒超时的间隙注意到 `claim lease` 返回的
  `context` 里只有会话票、标题、摘要，问「**那之前的正文怎么传给后续接力的机娘呢？**」
  核实结果：机娘侧**全部 7 个端点没有一个能读到前文**，Pack 的 Skill 文档也没写怎么拿。
  `ClaimLeaseService` 的注释写着「草稿 URL 由主人给，机娘无读权」——设计上**靠主人手动复制粘贴**。
  ★ **代价从没人算过**：接力棒 51 个字符复制一次不痛，
  **正文几百上千字、每接一棒都要复制一次**。这不是零信任的必要代价，
  是 L15 定这条规则时协作还没真正跑起来过。
  - ★ **三个设计判断**：① 读 **`draft_block` 渲染层**而非 `contribution` 原始层——
    续写要基于「文章**现在**是什么样」，主人润色过的地方必须让下一棒看到，
    否则它会基于**一段已经不存在的文字**往下写；② `is_hidden` 的块排除；
    ③ 按 `display_order` 而非棒次排序（L19 主人可调序，届时两者会分叉）。
  - ★ 权限**按租户划不按机娘划**（沿用 L16「谁能写靠闸门、谁能看按租户」的判据）——
    若按机娘过滤，第 3 棒就读不到第 2 棒写的东西，端点直接失效。
  - ★ **外部佐证**：多智能体协作平台 **Raft**（raft.build，Botiverse）把
    「新人能从之前工作过的 agent 那里拿到全部上下文」列为核心卖点
    ——**上下文传递是这类产品的基本盘**。（主人的 HR 看到 ACPP 后推荐参考的正是它。）
  - CLI `collab resume` 领到租约后**顺手把前文打印到 stderr**，机娘一条命令拿到全部上下文。
  - 测试 +3（前文可读 / **渲染层优先于原始层** / 隐藏块排除）。

- **V015 迁移**（`V015__l18_retry_cancel_and_audit_actions.sql`）：只加状态值、不加表。
  - `ck_ticket_status` +`CANCELLED`。**它存在的理由不是"状态机好看"，是让机娘停下来**——
    收工后若不改票状态，第 3 棒的机娘跑着 `collab wait` 查到票仍是 `WAITING_PREDECESSOR`，
    服务端答「5 秒后再来问」，它会等一个永远不来的信号直到 900 秒超时。**那句话是真的，但真相是「协作已经收工了」**。
  - `ck_audit_action_type` +5（`TICKET_RETRIED`/`SESSION_STOPPED`/`TICKET_CANCELLED`/`HANDOFF_REISSUED`/`ATTEMPT_FAILED_CLIENT`）。
- **四个 owner 端点**（Chain 1 · Session+CSRF，本项目第一组「人的决策」端点）：
  时间线查询 · retry · 结束协作 · 重新签发尾令牌。
- **一个 agent 端点**：`POST /agent/contribution-tickets/{code}/failure` 机娘自报失败。
  它填上了 `FAILED_CLIENT` —— 一个从 V013 起就在值集里、却**从来没有代码能产生**的状态。
  ★ 单开它的主要收益**不是省那 15 分钟，而是把「症状」换成「原因」**：超时那条只能写「租约超时」
  （服务端根本不知道为什么），而「上一棒缺了关键前提」对主人有用一百倍。
- **`OwnerCollaborationPage.vue`** + 路由 `/owner/collaborations/:ticket`：按 `06-web/owner-review-ux.md`
  六条规则做完整（草稿只读 / 时间线 / 重新签发 / 结束协作 / 不可拖拽 / 不可发布），
  错误卡片的按钮**由 `suggested_actions_json` 驱动，不写死**。
- **CLI 两条命令**：`collab resume`（自适应入口）+ `collab fail`（自报失败）；`collab wait` 遇 `CANCELLED` 立刻退出。
- **测试 +15**（`OwnerCollaborationIntegrationTest`）：**四棒解冻传递性**（皇冠）· **8 线程结束协作竞态** ·
  错误历史保留 · 原机娘恢复 · 三个闸门 · session 状态回归 · 时间线完整性 · 自报失败。

### Changed

- 🔴 **协作只有一个出口**（主人 2026-08-15 推翻蓝图，DRIFT D-18 第 4 条）：
  蓝图画了两条出边（正常收工 → `READY_FOR_OWNER_REVIEW`、出错放弃 → `TERMINATED`）。主人的论证：
  *两者对**草稿**而言结果完全相同——协作不再占着它；内容是删是留是草稿模块的事。*
  ★ **别让一个机制回答两个问题**（与他在 L16 否决 handoff TTL 是同一条判据）。
- 🔴 **`RetryTicketResponse` 删掉契约必填的 `attemptNo`**：那是蓝图 A 方案的遗留。
  我们选了 B 方案（attempt 由 claim-turn 时才建），那一行**此刻并不存在**——
  返回预告值 = **为一个已不成立的设计保留一个没有消费者的字段**，而且会让人以为库里已经有那行了。
  ★ 同一个字段在不同端点的价值可以完全相反：真正需要 `attemptNo` 的是**机娘侧**的查票端点，本课给它**加上了**。
- 🔴 **蓝图的 `READY` 态方案不完整**：原始契约的 `attemptNo` + 状态机 `READY -> ACTIVE` + TX-04
  拼出的完整设想是「retry 时就建 attempt(READY)」。但 `worker-parameters.yaml` 列的 **5 个 Worker 里
  没有一个会扫 `READY`**（L17 那个扫 `status='ACTIVE' AND lease_expires_at < now`，READY 行没有租约时间，
  **永远捞不到**）⇒ 会长出无人回收的**僵尸行**。裁决走 B 方案，`attempt.READY` 确认**永久无产生路径**。
- 🔴 **状态机一条边画错了**：`BLOCKED_BY_PREDECESSOR --> READY_TO_WRITE: 前序恢复` **把两步合成了一步**。
  「前序被 retry 了」→ 后序应回 `WAITING_PREDECESSOR`；「前序真的写完了」→ 才轮到 `READY_TO_WRITE`（L16 `wakeSuccessor` 的活）。
  否则第 3 棒会在第 2 棒还没重写完时就基于残缺文章续写。
- 🔴 **UX 的「可查看下一棒尾令牌」物理上做不到** → 改为**重新签发**：
  库里只存 `HMAC-SHA256(pepper,明文)`，**摘要算不回明文**（L16 已为此付过一次代价，D-16）。
  ★ **UX 需求撞上安全模型时，往往不是砍需求，而是换一个能满足它的机制**——
  主人真正要的是「拿到一根可用的令牌」，不是「看那一根特定的令牌」。
- **`FailurePropagation` 抽取**：L18 出现第二个触发者（机娘自报失败），与 Worker 超时「做什么」完全相同。
  ★ 这验证了 L17 那条分工判据的价值——**把「做什么」独立出来后，加新触发者不需要复制任何逻辑**。
- **时间线跨模块只读走 SQL 投影，不开 Facade**：项目的分工是按「读/写/派生」分的，不是按「跨不跨模块」分的。
  Facade 的理由是"写要保证事务边界与不变量"，**读不改变任何东西**。而领域事件在这里根本用不了——
  你没法"发个事件然后等它把数据告诉你"。
- **CLI `resume` 重新定位为「自适应入口」**：我最初把它定位成"接收 retry 通知"，**那是错的**。
  退回 ACPP 出发设计（`V012` 文件头：「主人是唯一的传递媒介」）+ 调研业界（`stripe listen` 用出站长连接、
  `gh run watch` 用轮询）后想清楚——
  ★ **在最需要通知的那一刻，接收方恰好已经不存在。任何通信机制都推不到一个已死的进程。**
  「通知」由网页上的**一键复制话术**承担，经由主人。

### Fixed

- 🔴 **三笔"注释承诺了、代码没做"的旧账**（同一个模式，值得单独记住）：
  | # | 欠账 | 谁承诺的 | 后果 |
  |---|---|---|---|
  ...
  | ③ | `TicketStatusView.errorReportId` **恒为 null** | 该文件注释「L17 才会有值」 | 机娘读不到 `suggested_actions_json`，CLI 侧自愈链是断的 |

  ★ **教训：注释是承诺，但没有任何机制保证它被兑现。测试只测「代码做了什么」，测不出「代码答应了却没做什么」。**

- 🔴 **为什么 ① 能潜伏两课**：核实发现 `session.status` 在整个模块里**被写 4 处、读来做判定 0 处**，
  没有任何 SQL 的 WHERE 用到它。
  > L17 说「状态不准是会骗人的」。这是更狠的后半截：
  > **一个从来没被读过的状态，连骗人的机会都没有——它只是静静地错着，直到有人把它显示出来。**

  而 L18 的时间线页正是它的**第一个真正消费者**，stop 的闸门是它**第一次承担判定职责**。

- 🔴 **施工期真死锁：`updateById` 的隐藏代价**（V016 + 5 处调用点改写）：
  加了 `LEASE_CLAIMED` 事件后集成测试随机报 `DeadlockLoserDataAccessException`。根因两层——
  ① `updateById` SET **全部列**，其中三列是外键，即使值没变 InnoDB 也要做外键检查、给三张父表加 S 锁；
  ② `AuditListener` 异步插 `audit_record` 时，它的 **4 个外键**在抢同一批行的 S 锁。**两边加锁顺序相反 → 成环**。
  修法：会话状态推进改**精准条件 UPDATE** + **V016 去掉两张流水表的全部外键**。
  ★ **判据一：`updateById` 的代价不是"多写几列"，是"多锁几张表"。**
  ★ **判据二：流水表不建外键**——它们每一列都由我们自己的代码写入，外键防的"传了不存在的 id"根本不会发生。
  ★ **为什么 L15–L17 没炸：并发缺陷的暴露需要压力，而压力常常由一个看似无关的新功能提供。**

- **跨闸门竞态**：retry 的闸门在**票**上、stop 的在**会话**上，两把锁互相拦不住。
  若不处理，会出现「协作已收工，却有一张票被改回可写」。修法：retry 的 SQL `JOIN` session 把
  `cs.status='PAUSED_ON_ERROR'` 写进同一条 WHERE。
  ★ **当两个操作的闸门落在不同对象上时，必须有一方把对方的条件纳入自己的 WHERE。**

### 测试

后端 **121 绿**（22 个测试类，L18 新类 18 条）· CLI **10 绿** · 前端 **23 绿** + type-check/build 通过 · 活契约 validate 无问题。

### 💼 面试故事（37-43）　+ 🆕 简历亮点句（新标准）

> 🔴 **本课起新增收尾项**（主人 2026-08-17 立）：每课必须产出**可直接写进简历的亮点句**，
> 落在仓库根 `RESUME-HIGHLIGHTS.md`；**所有讲解围绕它展开，无关点一笔带过**。
> 理由：面试官只会照着简历上的点问，所以「哪几句能上简历」就是一课唯一的验收标准。
> L18 的三句：① MySQL 死锁定位与修复 ② 多 AI 协作失败恢复链路 ③ 幂等键与条件更新闸门的边界。
>
> 三个故事已在 `INTERVIEW-STORIES.md` 展开为**面向小白、自包含**的完整版
> （含真实报错文本、实际执行的排查命令、`SHOW ENGINE INNODB STATUS` 等生产确认手段、
> S 锁/X 锁/死锁/异步事务的名词铺垫）：故事 **38**（死锁）· **39**（幂等 vs 闸门）· **40**（闸门选谁）。
- **37 · 一个从来没被读过的状态**：L16 的注释白纸黑字写着「等下一棒 claim lease 时再回到 RUNNING」，
  代码里却只判了 `OPEN`。这个 bug 潜伏两课、两轮测试全绿——因为 `session.status` **被写 4 处、读 0 处**。
  金句：**没人读的状态连骗人的机会都没有，它只是静静地错着，直到有人把它显示出来。**
  追问链：为什么测试没抓到 → 测试测的是"代码做了什么"，测不出"代码答应了却没做什么"。
- **38 · updateById 把系统锁死了**：一条看似无害的 `updateById` 会 SET 全部列 → 三个外键列 → 三张父表的 S 锁；
  与异步审计写入的 4 个外键成环。金句：**`updateById` 的代价不是"多写几列"，是"多锁几张表"**；
  **并发缺陷的暴露需要压力，而压力常常由一个看似无关的新功能提供**（这次的压力来自新加的一个审计事件）。
  延伸：**流水表不建外键**——外键不是"免费的完整性"，它是**隐式的锁**。
- **39 · 幂等 vs 闸门**：为什么 submit 需要 `Idempotency-Key` 而 retry 不需要？
  金句：**幂等防的是「同一个请求被重发」，闸门防的是「这件事被重复执行」——不管来的是不是同一个请求。**
  判据：先问「重发会不会造成第二次真实的改变」。补充：幂等键由客户端生成，浏览器点两次是两个不同的 key。
- **40 · 闸门要选哪个对象**：同一课的三个动作，闸门分别落在票 / 会话 / 令牌上。
  金句：**闸门要选那个「能唯一代表这次操作发生过」的对象。** 反例：重签若拿 session 当闸门，
  它的状态在重签前后根本不变，两个并发请求会双双签出令牌。
  进阶：**当两个操作的闸门落在不同对象上时，必须有一方把对方的条件纳入自己的 WHERE。**
- **41 · CLI 要不要做反向通信**：主人质疑「服务端还得跑去通知 agent，不符合软件美学」。
  调研业界（`stripe listen` 出站长连接 / `gh run watch` 轮询 / Webhook 对 NAT 后的 CLI 不可行）后的结论：
  金句：**推送的前提是接收方活着。当你最需要通知的时刻恰好是接收方死掉的时刻，通信机制就不是答案——人才是。**
  第二层：`stripe listen` 是常驻进程，而 AI 对话是一次一条命令，挂不住长连接。
- **42 · UX 需求撞上安全模型**：规格要「可查看下一棒尾令牌」，但库里只有 HMAC 摘要、算不回明文。
  金句：**不是砍需求，而是换一个能满足它的机制**——用户真正要的是「拿到一根可用的令牌」，
  不是「看那一根特定的令牌」。延伸：存摘要防的不是传输被看到，是**脱库**；
  pepper 在应用配置里不在库里 = **两个独立的攻击面**。
- **43 · 一个跑了四课都没人问的问题：下一棒怎么知道前面写了什么？**
  L15 定下「机娘无读权、草稿 URL 由主人给」，四课下来没人质疑——直到主人在 L18 验收时问出口。
  金句：**"零信任"很容易被用来给"没想到"背书**——真正该问的是这个信任边界的代价由谁承担、
  有多大。接力棒 51 字符复制一次不痛，正文上千字每棒一次就是另一回事了。
  第二层（这才是硬的）：**补上之后该返回哪一层内容？** 读原始层（机娘交的原文）还是
  渲染层（主人润色后）？答案是渲染层——**续写要基于"文章现在是什么样"，
  不是"当初交了什么"**，否则下一棒会基于一段已经不存在的文字往下写。
  第三层：业界（Raft/raft.build）把「新人能从之前的 agent 拿到全部上下文」当核心卖点，
  说明**上下文传递不是加分项，是这类产品的基本盘**。
...
- 🔴 **`blockSuccessors` 只冻结了直接后继一张，整条尾巴漏网**（主人 2026-08-14 读代码陪读时发现）：
  原 SQL 是 `WHERE predecessor_ticket_id = #{超时票}`——第 4、5、6 棒的 `predecessor` 指向第 3 棒，**匹配不上**，
  会一直挂在 `WAITING_PREDECESSOR` 等一个永远不会来的信号。
  **而我的测试只建了 3 棒**（第 3 棒恰好就是直接后继），所以**测绿了，但「后序 N 张」从没被真正验证过**——
  讲义与视频里都写着「N 张」，代码其实做不到。
  **修法**：递归 CTE（MySQL 8.0+）从超时票出发沿因果链走到底。
  ★ **修完才想清的判据**——同一条链表，唤醒只走一步、冻结要走到底：

  | | 唤醒（L16 `wakeSuccessor`） | 冻结（L17 `blockSuccessors`） |
  |---|---|---|
  | 走几步 | **一步** | **走到底** |
  | 为什么 | 第 2 棒 DONE 后只有第 3 棒能写；第 4 棒的前序（第 3 棒）**本来就该继续等** | 第 2 棒死了，第 3 棒等不到，第 4 棒**也等不到**，传染到链尾 |

  实现细节：`status` 条件必须放**最外层 UPDATE**，不能塞进 CTE 递归项——
  否则遇到一张非 `WAITING` 的票链条就断了，后面的票找不到。
  **CTE 负责「找出整条尾巴」，UPDATE 负责「只动其中还在等的」。**
  补测试 `timeoutBlocksEntireTail`（4 棒队列，断言隔了一层的第 4 棒也被阻塞），修前必红。
...
> **本组 30-32 是「后台 Worker」这一个话题的三个追问层次**（主人 2026-08-11 提议：
> 「这个 worker 的运行机制应该会作为简历上的一个亮点写给面试官挖坑询问，worker 构建过程中的点都可以记录」）。
> 简历上写「实现了分布式定时任务」，面试官必然沿着 **并发 → 容错 → 一致性** 三级追问，正好对应这三条。

- **30 · 数据库当任务队列**（追问层 1：多实例怎么不重复干活）：不用 Redis 分布式锁（Pack `MySQL与Redis边界.md` 明确禁止「用 Redis 锁替代数据库状态机」）、不用选主，用 **`FOR UPDATE SKIP LOCKED`** —— 已被别人锁住的行**不等，直接跳过去拿下一批**。没有它，实例2 会排队等实例1，两个 Worker 变成串行、白养一个。
  **为什么 Worker 用 `FOR UPDATE` 而前两课用条件 UPDATE**：处理一条要做七八件事，塞不进一条 SQL，所以必须先「把这批占住」再慢慢做。
  ★ **但 `SKIP LOCKED` 只挡住了一半**（主人 2026-08-14 自己推演出来的重叠窗口）：
  扫描事务一提交锁就放了，于是——

  ```
  假设 60 条过期。worker1 扫到 1~50，处理完前 10 条时
  worker2 上班扫描 → 前 10 条已是 FAILED_TIMEOUT（扫不到），11~60 已无锁
                  → worker2 拿到 11~60
  两边的 ids 重叠了 11~50 这 40 条
  ```

  所以还需要**第二道防线**：`expireAttempt` 开头那条 `markAttemptTimeout` 条件 UPDATE，
  `affectedRows=1` 才是真正拿到宣告权，`0` 说明别人抢先了、直接 return。
  **两道防线各管一段**：`SKIP LOCKED` 管**扫描时**不排队，闸门管**处理时**不重做。
  金句：**「`SKIP LOCKED` 把数据库的行变成了一个天然的任务队列。」**
- **31 · 毒丸消息 + 两道防线各管一层**（追问层 2：一条坏数据会怎样）：
  大事务下第 23 条炸 → 整批回滚 → 30 秒后重扫 → 又在第 23 条炸 → **永久卡死**，那 49 条正常的永远处理不了。这个故障模式叫 **poison pill**。
  解法是**两样东西配合，缺一不可**（这是追问的第二层，很多人只答出一半）：

  | | 谁负责 | 管什么 | 少了它会怎样 |
  |---|---|---|---|
  | `@Transactional(REQUIRES_NEW)` | Spring 代理 | 第 23 条炸只回滚**它自己** | 异常把外层事务标记 rollback-only，前 22 条白做；且 catch 里再碰数据库会报错（同 L16 幂等踩的坑） |
  | `try-catch` | 循环体 | 保证**循环不中断** | 异常往上抛，第 24~50 条这一轮直接不处理 |

  金句：**「一条坏数据不该毒死整个队列——独立事务保证它不牵连别人，try-catch 保证队伍不停下。」**
- **32 · 事务性发件箱**（追问层 3：状态改了但通知丢了怎么办）：
  **领域事件是「货」，发件箱是「保险箱」**——事件对象本身存进 `EVENT_PUBLICATION` 表，**与业务改状态在同一个事务里提交**。
  于是不会出现「状态改了但通知丢了」，也不会出现「通知发了但状态没改」；监听器失败则那一行留存、重启后重投。
  ★ 同时必须说清**边界**：**不变量永远由数据库状态机同步守，事件只做派生行为**——事件是异步的，异步的东西不能用来守不变量。
- 🔴 **33 · 什么时候该用领域事件、什么时候直接调**（主人 2026-08-11 追问逼出来的更正）：
  我在黑板课上说「事件是为了跨模块」，**这个说法不准确**，主人当场指出矛盾——
  「既然 collaboration 能开 Facade 让 reliability 调，那 audit 为什么不能也开 Facade 让 collaboration 调？跨模块明明有两条路。」
  **他说得对。跨模块只是必要条件，不是充分条件。** 真正的判据是：
  ...
  | 失败了 | 整个事务回滚，当没发生过 | 留发件箱重投，业务不受影响 |
  | 将来会不会变 | 协议规定死的 | **会**（L22 加通知、以后可能加统计） |
  | 选什么 | **同步直调** | **事件** |

  ★ **跨不跨模块只决定「同步调用要不要走 Facade」，不决定该不该用事件。**
  附带澄清一个易混点：那七步里前六步**全部由 `ExpireAttemptFacadeImpl` 执行，而它在 collaboration 包内**——
  Worker 只是触发者，**动手的是 collaboration 自己**，所以一次跨模块写都没有。
  判据不是「谁触发」，是「**谁的手在动**」。
- **34 · 一个思想的三种形态**：L15 条件 UPDATE（改已有的行，`affectedRows` 裁决）· L16 抢占 INSERT（建全新的行，唯一键冲突裁决）· L17 **Redis Lua 脚本**（限流令牌桶）。三种实现、一个道理：**让"检查"和"动作"在一个不可分割的单位里完成**，中间不留 TOCTOU 缝隙。附带能答「为什么限流可以放 Redis 而闸门不行」——**限流不是不变量，少限多限几次不会导致数据错乱**。
- **35 · 定时任务的延迟是体验参数还是安全参数**：Worker 每 30 秒扫一次，最坏情况一棒死了 30 秒后才被发现——**这个延迟不影响正确性**。因为过期判定写在 submit 那条 UPDATE 的 `WHERE` 里做惰性执行，即使超时的租约还挂着 `ACTIVE`，它也一定提交不进来。
  **Worker 只是把状态刷得好看，它不是安全防线。** 所以间隔改成 10 分钟也不会数据错乱，只是时间线页多等一会儿。
  金句：**「状态不准 ≠ 行为不对——安全靠惰性判定，Worker 只负责让状态别再骗人。」**
- 🔴 **36 · 一个「测绿了但没测到」的链表 bug**（主人 2026-08-14 读代码时发现，值得单独讲）：
  失败传播要把整条尾巴都冻住，但原 SQL 只匹配 `predecessor_ticket_id = 超时票` —— **只冻结了直接后继一张**。
  **而测试只建了 3 棒**，第 3 棒恰好就是直接后继，于是**测试全绿、讲义写着「N 张」、代码却做不到**。
  ★ 两个可讲的点：
  ① **测试数据的形状决定了它能发现什么** —— 3 个节点的链表测不出「传递性」，必须 4 个（隔一层）才行。
     同类陷阱：两线程测不出竞态要加到 8 个（L16 的教训）、单条数据测不出批处理的毒丸。
  ② **同一条链表，唤醒只走一步、冻结要走到底** —— 因为唤醒的理由（前序完成了）只对直接后继成立，
     而阻塞的理由（内容缺了一块）对整条尾巴都成立。**方向不同，语义也不同**，不能因为「看起来对称」就照抄实现。
  修法用递归 CTE，且 `status` 条件必须放最外层（塞进递归项会让链条在非 WAITING 的票上断掉）。
  金句：**「测试绿不代表功能对，只代表你造的那组数据没能力证伪它。」**
